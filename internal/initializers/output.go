...
func VulnerabilityOutput(scanName string, severity string, url string, summary string) {
	...
	switch severity {
	case "Critical":
		fmt.Printf("[%s] [%s] %s \n", magenta(scanName), magenta(severity), url)
	case "High":
		fmt.Printf("[%s] [%s] %s \n", red(scanName), red(severity), url)
	case "Medium":
		fmt.Printf("[%s] [%s] %s \n", yellow(scanName), yellow(severity), url)
	case "Low":
		fmt.Printf("[%s] [%s] %s \n", green(scanName), green(severity), url)
	case "Info":
		fmt.Printf("[%s] [%s] %s \n", blue(scanName), blue(severity), url)
	}
